...
Event.stop(event);}
function insertMention(){var lPad=' ',rPad=' ',name=popup.getSelectedName(),offset=keyCache.getOffset(),mention,prevChar;if(!name){if(!popup.spinnerIsVisible()){popup.hide();}
return;}
...
getCaret();mention=lPad+'@"'+name+'"'+rPad;textarea.value=textarea.value.slice(0,offset-1)+mention+textarea.value.slice(offset+keyCache.getLength());setCaret(offset+mention.length-1);popup.hide();}
