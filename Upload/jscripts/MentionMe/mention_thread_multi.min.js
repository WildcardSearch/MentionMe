var MentionMe=(function(m){var spinner;function init(){var post_ids,mentioned=Cookie.get("multi_mention");if(mentioned){post_ids=mentioned.split("|");post_ids.each(function(post_id){if($("multi_mention_"+post_id)){element=$("multi_mention_"+post_id);element.src=element.src.replace("postbit_multi_mention.gif","postbit_multi_mention_on.gif");}
...
return true;}
function mention(pid){var new_post_ids=new Array(),mentioned=Cookie.get("multi_mention"),is_new=true,post_ids;if(mentioned){post_ids=mentioned.split("|");post_ids.each(function(post_id){if(post_id!=pid&&post_id!=''){new_post_ids[new_post_ids.length]=post_id;}else if(post_id==pid){is_new=false;}});}
...
$(id).focus();}
function clear(){var post_ids,mentioned=Cookie.get("multi_mention");$('quickreply_multi_mention').hide();if(mentioned){post_ids=mentioned.split("|");post_ids.each(function(post_id){if($("multi_mention_"+post_id)){element=$("multi_mention_"+post_id);element.src=element.src.replace("postbit_multi_mention_on.gif","postbit_multi_mention.gif");}
