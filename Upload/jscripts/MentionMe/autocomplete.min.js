...
c=this.nameCache.getData(),e=-1!==navigator.userAgent.toLowerCase().indexOf("msie")?"hand":"pointer";this.items=this.nameCache.getItems();if(0===d&&0==this.spinnerVisible)0<this.keyCache.getLength()?(this.clear(),this.$body.html(b("<span/>",{"class":"mentionme_typed_text"}).html(this.keyCache.getText()))):this.showInstructions();else for(this.clear(),a=0;a<d;a++){var k=this.items[a];var l=c[k].username;if(this.keyCache.getText()){var g=k.indexOf(this.keyCache.getText());if(f.fullText&&-1!==g||!f.fullText&&
...
typeof CKEDITOR.instances){var a=b.map(CKEDITOR.instances,function(a,b){return b})[0];if("object"!==typeof CKEDITOR.instances[a])return!1;new u(a)}else if(null!==MyBBEditor&&"object"===typeof MyBBEditor&&MyBBEditor.getRangeHelper&&"function"===typeof MyBBEditor.getRangeHelper)new A;else if(0<b("#message").length||0<b("#signature").length){b("#message").length?a="message":b("#signature").length&&(a="signature");if(!a||!b("#"+a).length)return!1;new r(a)}c.length&&(c.prop("id","dvz_shoutbox_input"),
new r("dvz_shoutbox_input"));b(".quick_edit_button").click(m);b("#quick_reply_submit").click(w)});t.autoComplete={setup:function(c){b.extend(v,c.lang||{});delete c.lang;b.extend(f,c||{});b("minLength maxLength maxItems fullText showAvatars lockSelection".split(" ")).each(function(){"undefined"!==typeof f[this]&&(f[this]=parseInt(f[this],10))});f.defaultAvatar=f.imageDirectory+"/default_avatar.png"}};return t}(jQuery,MentionMe||{});