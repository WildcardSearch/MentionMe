var MentionMe=(function($,m){"use strict";var maxItems=5,options={minLength:3,maxLength:30,},lang={loading:"loading . . .",instructions:"type a user name",},textAreaPadding=2,cursorPointer="pointer",items=[],selection={start:0,end:0,},key={BACKSPACE:8,NEWLINE:10,ENTER:13,SHIFT:16,CTRL:17,ALT:18,ESC:27,SPACE:32,PAGE_UP:33,PAGE_DOWN:34,END:35,HOME:36,LEFT:37,UP:38,RIGHT:39,DOWN:40,DEL:46,},$textarea,$container,$input,$inputDiv,selectionRange,positioner,cache={},popup={},keyCache={},d={log:function(){},};function setup(opt){$.extend(lang,opt.lang||{});delete opt.lang;$.extend(options,opt||{});}
...
function onKeyUp(e){var proceed=true;getCaret();if(!popup.isVisible()){$([key.LEFT,key.RIGHT,key.UP,key.DOWN,key.BACKSPACE,key.ESC,key.SHIFT,key.CTRL,key.ALT]).each(function(){if(e.keyCode==this){proceed=false;}});if(proceed&&$textarea.val().slice(selection.start-1,selection.end)=="@"){popup.show();}}}
function onKeyDown(e){switch(e.keyCode){case key.ENTER:insertMention();break;case key.UP:popup.select("previous");break;case key.DOWN:popup.select("next");break;case key.END:popup.select("last");break;case key.HOME:popup.select();break;case key.PAGE_UP:popup.select("previousPage");break;case key.PAGE_DOWN:popup.select("nextPage");break;case key.ESC:popup.hide();break;case key.BACKSPACE:if($input.val()===""){popup.hide();}
return;break;default:return;}
e.preventDefault();}
function insertMention(){var name=popup.getSelectedName(),offset=keyCache.getOffset(),quote="";if(!name){if(!popup.spinnerIsVisible()){popup.hide();}
return;}
...
name+
quote+
$textarea.val().slice(offset));setCaret(offset+name.length+2);popup.hide();}
...
keyCache=(function(){var data="",mirror="",offset=0;function init(){data="";mirror=""
offset=selection.start;}
function update(){var ret=false;if(data!==$input.val()){ret=true;}
data=$input.val();return ret;}
function checkCaret(code){if($input.val().length>options.maxLength){popup.hide();}}
...
keyCache.init();popup.update();left=taCoords.left+
coords[0]+
5;top=(taCoords.top+
coords[1]+
getPageScroll()[1])-
lineHeight+
3;move(left,top);$mainDiv.show();lastSelected=null;select();visible=true;$div.mouseover(onMouseMove);$div.click(onClick);$(document).click(hide);$textarea.click(hide);$input.keydown(onKeyDown);$input.keyup(updateCheck);$input.focus();}
function hide(){$mainDiv.hide();$div.unbind("mouseover",onMouseMove);$div.unbind("click",onClick);$(document).unbind("click",hide);$textarea.unbind("click",hide);$input.unbind("keydown",onKeyDown);$input.unbind("keyup",updateCheck);visible=false;$input.val("");$textarea.focus();}
...
function getCurrentHeight(){return(lineHeight*Math.max(1,Math.min(5,items.length)))+2;}
